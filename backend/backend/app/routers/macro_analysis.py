...
# ============================================================================
# DATA FETCHING (Uses market endpoints)
# ============================================================================

async def get_quote_data(symbol: str) -> Optional[Dict]:
    """
    Fetch quote data - tries internal API first, falls back to defaults.
    Uses the market quote endpoint.
    """
    try:
        # Try to call the market quote endpoint
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"http://localhost:8000/api/v1/market/quote/{symbol}",
                timeout=5.0
            )
            if response.status_code == 200:
                ...
                return {
                    "quote": {
                        ...
                        "netPercentChangeInDouble": data.get("change_pct", 0),
                        "netChange": data.get("change", 0)
                    }
                }
