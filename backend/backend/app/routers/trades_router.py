...
class TradeRecordCreate(BaseModel):
    ...
    strategy: str = Field(
        default="ipmcc", 
        description="Strategy: ipmcc, 112, strangle, bwb, spread, other"
    )
    # Strategy category for filtering
    strategy_category: str = Field(
        default="swing",
        description="Category: swing (IPMCC/112/Strangle/BWB/Spread)"
    )
    ...
    regime: Optional[str] = Field(None, description="Market regime at entry")
    followed_rules: Optional[bool] = Field(None, description="Did trade follow strategy rules?")
